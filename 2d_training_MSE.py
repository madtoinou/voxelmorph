...
lambdas_param = [0.00005, 0.05, 0.5]
...
for lambda_param in lambdas_param:
    ...
    # train_generator = vxm.generators.scan_to_atlas(vols_names, atlas = slice_fixed, batch_size=16)

    checkpoint = tf.keras.callbacks.ModelCheckpoint("best_model2D_MSE" + str(lambda_param) + ".hdf5", monitor='val_loss', verbose=1,
    ...
    vxm_model.save_weights("wght_2d_MSE_"+str(lambda_param)+".keras")

    util.export_history(hist, "hist_2d_MSE_"+str(lambda_param)+".txt")
